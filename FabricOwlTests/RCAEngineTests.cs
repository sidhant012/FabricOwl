...
namespace FabricOwlTests
{
    ...
    {
        readonly RCAEngine rca = new();
        static readonly string config = File.ReadAllText(@"ConfigData\generatedConfig.txt");
        IEnumerable<ConcurrentEventsConfig>? testGenerateConfig = null;

        [TestInitialize]
        public void InitTests() 
        {
            Assert.IsTrue(JsonHelper.TryDeserializeObject(config, out testGenerateConfig));
        }
